Events.Subscribe("ivd_core:playerJoined", function(RocstarID)
    ...
    if Config.Server.ClosedServer and not IsAdmin then
        ...
        if #result > 0 then
            ...
            result[1].Weapons = IVD.JSON.Decode(result[1].Weapons)
            result[1].Health = IVD.JSON.Decode(result[1].Health)
...
Events.Subscribe("ivd_core:UpdatePlayerServerData", function(RocstarID, PlayerData)
    ...
    local source = Events.GetSource()
    MySQL.Execute("UPDATE players SET position=?, money=?, Items=?, Weapons=?, Health=? WHERE RockstarID=?", { IVD.JSON.Encode(PlayerData.position), IVD.JSON.Encode(PlayerData.money), IVD.JSON.Encode(PlayerData.Items), IVD.JSON.Encode(PlayerData.Weapons), IVD.JSON.Encode(PlayerData.Health), RocstarID }, function(affectedRows)
